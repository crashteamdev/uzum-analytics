...
--changeset vitaxa:add-title-photokey-review-to-product-daily-sales
DROP TABLE IF EXISTS uzum.uzum_product_daily_sales_mv;

ALTER TABLE uzum.uzum_product_daily_sales
    ADD COLUMN IF NOT EXISTS title AggregateFunction(anyLast, String);

ALTER TABLE uzum.uzum_product_daily_sales
    ADD COLUMN IF NOT EXISTS photo_key AggregateFunction(anyLast, String);

ALTER TABLE uzum.uzum_product_daily_sales
    ADD COLUMN IF NOT EXISTS reviews_amount AggregateFunction(anyLast, UInt32);

CREATE MATERIALIZED VIEW IF NOT EXISTS uzum.ke_product_daily_sales_mv
